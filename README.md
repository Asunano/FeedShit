...
- 审计日志自动清理（保留 90 天），防止日志表无限增长
- 配置值支持 DB → 环境变量 → 代码默认值三级回退

## 部署须知

### 水平扩展限制

FeedShit 当前设计为**单实例部署**，以下组件为进程级内存存储，多实例部署时**不会共享状态**：

| 组件 | 类型 | 影响 |
|------|------|------|
| Session 管理器 | 内存 map | 多实例间会话不共享，用户可能被随机登出 |
| CSRF/Nonce 缓存 | 内存 map | 多实例间 CSRF token 校验失败 |
| IP 限速器 (RateLimiter) | 内存计数器 | 限速重置，攻击者可轮询各实例 |
| 登录暴力破解追踪 | 内存计数器 | 锁定状态不共享 |
| API Token 每小时计数器 | 内存 map | 限额重置 |

> 如果未来需要水平扩展，建议引入 Redis 作为共享状态层，或将相关组件改为数据库持久化。
